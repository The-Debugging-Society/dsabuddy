...
export function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      try {
        localStorage.setItem('dsabuddy_dashboard_cache', JSON.stringify(updatedCache));
      } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (cachedData) {
        ...
        if (parsed.companies) {
          const companiesArray = Array.isArray(parsed.companies) ? parsed.companies : (parsed.companies.companies || []);
          setCompanies(companiesArray);
        }
      }
    } catch (e) {
  ...
  const renderSection = () => {
    switch (activeSection) {
      case 'dashboard':
        return <Dashboard user={user} platforms={platforms} analytics={analytics} onUpdate={fetchData} />;
      case 'problems':
